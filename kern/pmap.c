...
boot_alloc(uint32_t n)
{
	...
	//note it is unknow what would happen if more than 4mb is requested
	//since only 4mb is have page table mapping
	//note since npages is equal to total number of page in physical memory
	//and I chose to ignore base memory, if the physical memory is low
	//boot_alloc might panic even if there is enough memory in base memory
	if(n==0){
		return nextfree;
	}
	page_requested=PGNUM(ROUNDUP(n,PGSIZE));
	i386_detect_memory();
	if((npages-PGNUM(nextfree))>=page_requested){
		result=nextfree;
		nextfree+=page_requested*PGSIZE;
		return result;
	}
	else{
		panic("not enough continuous physical memory(did not check base memory)");
	}
...
mem_init(void)
{
    ...
    */
	...
	//For later processors You can set the PTE_PS flag and enable 4mb pages
	//the address frame of directory entry should be base of 4mb page if enabled.
	boot_map_region(kern_pgdir,KERNBASE,npages*PGSIZE,0,PTE_W);
	for(size_t i=PDX(KERNBASE);i<1024;i++){
		kern_pgdir[i]|=PTE_W|PTE_P;
	}
...
page_init(void)
{
	...
	size_t i;
	for (i = 0; i < npages; i++) {
		// first page of physical mem contain interrupt vector table for
		...
		// the address right after array of PageInfo rounded up in conidered free.
...
page_alloc(int alloc_flags)
{
	...
	if(alloc_flags & ALLOC_ZERO){
		byte_pointer=page2kva(free_page);
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref!=0){
		panic("trying to free page still referenced");
	}
...
{
	// Fill this function in
	uintptr_t dir_index=PDX(va);
	uintptr_t pg_index=PTX(va);
	pde_t *de=(pgdir+dir_index);
	physaddr_t pgtbl_p;
	pte_t *pgtbl_v;
	pte_t *pe;
	physaddr_t *pg;
	struct PageInfo *ptb_info=NULL;
	...
	if((PTE_P&*de)==0){
		...
		if((ptb_info=page_alloc(1))==NULL){
			return NULL;
		}
		ptb_info->pp_ref+=1;
		pgtbl_p=page2pa(ptb_info);
		*de=(pte_t)pgtbl_p|PTE_P;
	}
	else{
		pgtbl_p=(PTE_ADDR(*de));
	}
	pgtbl_v=KADDR(pgtbl_p);
	pe=(pgtbl_v+pg_index);
...
{
	physaddr_t pg;
	pte_t *pe=pgdir_walk(pgdir,va,true);
	if(pe==NULL){
		return -E_NO_MEM;
	}
	
	//This basically solves the corner case as it holds onto the page 
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if((pe=pgdir_walk(pgdir,va,false))==NULL){
		return NULL;
	}
	*pte_store=pe;
	if((*pe&PTE_P)==0){
...
page_remove(pde_t *pgdir, void *va)
{
	pte_t *pe;
	struct PageInfo *pg_info;
	pg_info=page_lookup(pgdir,va,&pe);
	if(pg_info==NULL){
		return;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for(uintptr_t i=start;i<end;i+=PGSIZE){
        if((entry=pgdir_walk(env->env_pgdir,(void *)i,true))==NULL||((*entry)&(perm|PTE_P))!=(perm|PTE_P)){
            if(i<(uintptr_t)va){
